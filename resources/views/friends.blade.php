        ...
        <section class="mt-10">
            ...
            <ul class="mt-4">
                @forelse($pendingRequests as $pending)
                    <li class="flex justify-between items-center p-4 border-b border-gray-600 bg-gray-800 rounded-md hover:bg-gray-700 transition duration-200">
                        <a href="{{ route('profile.show', $pending->id) }}" class="text-blue-400 font-semibold hover:underline">{{ $pending->name }}</a>
                        <div class="flex space-x-2">
                            <form method="POST" action="{{ route('friends.accept', $pending->id) }}">
                                @csrf
                                <button type="submit" class="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-4 rounded-lg" aria-label="Accept Friend Request from {{ $pending->name }}">
                                    {{ __('✅ Accept') }}
                                </button>
                            </form>
                            <form method="POST" action="{{ route('friends.reject', $pending->id) }}">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-4 rounded-lg" aria-label="Reject Friend Request from {{ $pending->name }}">
                                    {{ __('❌ Reject') }}
                                </button>
                            </form>
                        </div>
